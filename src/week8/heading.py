...
class Sentiment:
    ...
    def extract_words(self, document):
        return set(
            word.lower() for word in nltk.word_tokenize(document) if any(c.isalpha() for c in word)
        )
    ...
    def load_data(self):
        ...
        for filename in self.file_name:
            with open(os.path.join(self.directory, filename), encoding="UTF-8") as f:
                self.result.append([self.extract_words(line) for line in f.read().splitlines()])
        return self.result

    def generate_features(self, documents, label):
        features = []
        for document in documents:
            features.append(({word: (word in document) for word in self.words}, label))
        return features

    def classify(self, classifier, document):
        document_words = self.extract_words(document)
        features = {word: (word in document_words) for word in self.words}
...
class TermFrequency:
    ...
    def load_data(self):
        ...
        for filename in os.listdir(self.directory):
            with open(os.path.join(self.directory, filename), encoding="UTF-8") as f:
                # Extract words
                contents = [word.lower() for word in nltk.word_tokenize(f.read()) if word.isalpha()]
...
class Search:
    ...
    def find_templates(self, examples, corpus):
        ...
        middles = {middle: middles[middle] for middle in middles if len(middles[middle]) > 1}
    ...
    def filter_templates(self, templates):
        return sorted(templates, key=lambda t: len(t["prefix"]) + len(t["suffix"]), reverse=True)
    ...
    def match_query(self, q1, q2, order, corpus):
        ...
        return [
            {"order": order, "prefix": result[0], "middle": result[1], "suffix": result[2]}
    ...
    def common_prefix(self, *s):
        ...
        return "".join(
            ch[0] for ch in itertools.takewhile(lambda x: min(x) == max(x), zip(*s, strict=False))
        )
...
class Vectors:
    ...
    def closest_words(self, embedding):
        distances = {w: self.distance(embedding, self.words[w]) for w in self.words}
