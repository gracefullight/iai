...
class GridMDP(MDP):
    ...
    def to_grid(self, mapping):
        ...
        return list(
            reversed(
                [[mapping.get((x, y), None) for x in range(self.cols)] for y in range(self.rows)]
            )
        )
...
class POMDP(MDP):
    ...
    def __init__(
        self, actions, transitions=None, evidences=None, rewards=None, states=None, gamma=0.95
    ):
...
class Matrix:
    ...
    def matmul(A, B):
        ...
        return [
            [
                sum(ele_a * ele_b for ele_a, ele_b in zip(row_a, col_b, strict=False))
                for col_b in list(zip(*B, strict=False))
            ]
            for row_a in A
        ]
    ...
    def transpose(A):
        """Transpose a matrix"""
        return [list(i) for i in zip(*A, strict=False)]
